...
import { Collapse, Icon, Select, Row, Col, Checkbox, Button, Table, Empty, InputNumber, Divider, Slider, Modal, Input } from 'antd';
import $ from 'jquery';

import { SOPOONG_URL, SHOP_URL, SHOP_ID, MAIN_PATH, CREATIONS_PATH } from 'common';


export const CreationEditorListPanel = (props) => {
  ...
  const onWindowResize = () => {
    ...
    $("#creationListPanel").css("top", $('html, body').offset().top + 5);
    $("#creationListPanel").css("left", $('html, body').offset().left + $('#creationEditor_vrCanvas').outerWidth() - 480);
  ...
  const genAssetNameExtra = () => (
    <div id="creationEditor_Asset_Name">
    </div>
  );
    ...
    },
    {
      ...
      render: (text, record) => {
        if (record.type == "MODEL")
          ...
          return (
            <div>
              <span><InputNumber id="creationEditorAssemblyLevel" size="small" min={0} defaultValue={record.assemblyLevel} onChange={value => handleAssembleyLevelAsset(record, value)} ></InputNumber></span>
  ...
  const getCreationInfoMenu = () => {

    return (
      <>
      <Row type='flex' align='middle' justify='start' style={{ marginBottom: '0px'}}>
        <Col span={6} style={{paddingLeft: '10px', fontWeight: 'bold'}}>
            창작품명
        </Col>
        <Col span={16} style={{backgroundColor: '#ffffff', height: '40px'}}>
          <Input
            placeholder="창작품명을 입력하세요."
      ...
      <Row type='flex' align='middle' justify='start'   style={{ marginBottom: '7px'}}>
        ...
        <Col span={17} style={{backgroundColor: '#ffffff', height: '40px'}}>
          <Select value={visible} style={{ height: '40px', width: '100px' }} onChange={value => onVisible(value)}>
  ...
  return (
    <div>
      ...
      <div id="creationListPanel" className='vreditor-container' style={{ height: "100vh" }}>
        <Collapse
          defaultActiveKey={['2']}
        >
          ...
          <Panel header="레고 부품 목록"  key="3"  style={{ maxHeight: "30vh", overflow: "auto"  }} >

              <Row gutter={[8, 8]} style={{padding: '0px 10px'}}>
                {vrModelList ? (
                  vrModelList.map(vrModel => {
                    if(vrModel.vrModelType == 'function_block')
                      return (<></>)
                    else
                      return (
          ...
          <Panel header="선택하기" key="4" style={{ maxHeight: "30vh", overflow: "auto" }} >
            <Table
              columns={columns}
          ...
          <Panel header="조립단계보기"  key="5" style={{ maxHeight: "15vh", overflow: "auto" }}>
            <Row>
              <Col span={11}>
                <Slider
                  range
          ...
          <Panel header="설정하기" key="6" extra={genAssetNameExtra()}>
            ...
            <Row type="flex" justify="space-around" align="middle" gutter={[16, 16]} >
              <Col span={6}> 위치</Col>
              <Col span={6}><InputNumber id="creationEditor_Properties_Px" size="small" step={0.4} defaultValue={positionx} onChange={value => setPositionx(value)} onPressEnter={() => onTransformValueChange('position', 'x', positionx)}></InputNumber></Col>
              <Col span={6}><InputNumber id="creationEditor_Properties_Py" size="small" step={0.4} defaultValue={positiony} onChange={value => setPositiony(value)} onPressEnter={() => onTransformValueChange('position', 'y', positiony)}></InputNumber></Col>
              <Col span={6}><InputNumber id="creationEditor_Properties_Pz" size="small" step={0.4} defaultValue={positionz} onChange={value => setPositionz(value)} onPressEnter={() => onTransformValueChange('position', 'z', positionz)}></InputNumber></Col>
            </Row>
            <Row type="flex" justify="space-around" align="middle" gutter={[16, 16]} >
              <Col span={6}> 회전</Col>
              <Col span={6}><InputNumber id="creationEditor_Properties_Rx" size="small" step={15} defaultValue={rotationx} onChange={value => setRotationx(value)} onPressEnter={() => onTransformValueChange('rotation', 'x', rotationx)}></InputNumber></Col>
              <Col span={6}><InputNumber id="creationEditor_Properties_Ry" size="small" step={15} defaultValue={rotationy} onChange={value => setRotationy(value)} onPressEnter={() => onTransformValueChange('rotation', 'y', rotationy)}></InputNumber></Col>
              <Col span={6}><InputNumber id="creationEditor_Properties_Rz" size="small" step={15} defaultValue={rotationz} onChange={value => setRotationz(value)} onPressEnter={() => onTransformValueChange('rotation', 'z', rotationz)}></InputNumber></Col>
            </Row>
          </Panel>
          <Panel header="라이트"   key="7" >
