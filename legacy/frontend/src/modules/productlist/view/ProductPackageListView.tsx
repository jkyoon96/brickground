...
import { SOPOONG_URL, SHOP_URL, SHOP_ID, PRODUCT_PACKAGE_INFO_PATH, storageService  } from 'common';
import { ManualCardList, ProductCardList, ProductCard } from '.'

export const ProductPackageListView = (props) => {

  const params = queryString.parse(props.location.search);
  console.log('params : ' , params);

  const [checkedList, setCheckedList] = useState<string[]>(['패키지']);
  ...
  const genAddExtra = () => {
    ...
    if (storageService.getItem('brickground-user')) {
        ...
        if(user != undefined && user.role == 9) {
          return (<Link to={`${PRODUCT_PACKAGE_INFO_PATH}/0`} >상품 추가</Link>);
        }
    }
