...
export const ProductSetListView = (props) => {

  const params = queryString.parse(props.location.search);
  console.log('params : ' , params);

  const [checkedList, setCheckedList] = useState<string[]>(['브릭 낱개']);
  const [indeterminate, setIndeterminate] = useState<boolean>(true);
  const [checkAll, setCheckAll] = useState<boolean>(false);

  const [checkedSubjectList, setCheckedSubjectList] = useState<string[]>(['도트','베이스', '액자', '소품']);
  ...
  const subjectOptions = ['도트','베이스', '액자', '소품'];
