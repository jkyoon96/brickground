...
import { SOPOONG_URL, SHOP_URL, SHOP_ID, LOGIN_PATH, DOTARTS_PATH, DOTART_VIEW_PATH, DOTART_VIEW_2D_PATH, DOTART_INFO_PATH, DOTART_EDITOR_PATH, DOTART_EDITOR_2D_PATH, storageService } from 'common';
import { ConfirmModal } from 'components';
import { DotArtCommunityComponent, DotArtRemixCard, BlacklistAddDialog } from '.';

export const DotArtCommunityView = (props) => {
  ...
  const editCreation = () => {
    ...
    if(creation.setNames == '21226C')
      history.push(DOTART_EDITOR_2D_PATH + '/' + creation.creationId);
    else
      history.push(DOTART_EDITOR_PATH + '/' + creation.creationId);
  }
  ...
  const genDotArtType = () => {
    ...
    if(creation.setNames == '21226C')
      return (<span style={{fontWeight: 600}}> 2D </span>);

    return (<span style={{fontWeight: 600}}> 3D </span>);
  ...
  const genLinkImage = () => {
    ...
    if(creation.setNames == '21226C') {
      return (
        <Link to={`${DOTART_VIEW_2D_PATH}/${creation.creationId}`}>
          <img
    ...
    return (
      <Link to={`${DOTART_VIEW_PATH}/${creation.creationId}`}>
        <img
          id ={creation.creationId}
          src={creation.coverImage}
          style={{ width: '100%', height: '100%', maxWidth: '480px', maxHeight: '400px', margin: '0 auto', borderRadius: '10px' }}
        ...
        <Col md={8} xs={24}>
          ...
          <div style={{ maxHeight: '480px', borderLeft: '1px solid #d9dadb', padding: '20px 0px 30px 40px'}}>
            ...
            <div style={{marginTop: '5px'}}>
                {getHtml('dotart-description', creation.description)}
                <div id='dotart-description' className='ck-content' style={{width: '95%', height: '300px', overflow: 'auto', backgroundColor: 'rgba(77, 151, 255, 0.1)'}}/>
            </div>
          </div>
