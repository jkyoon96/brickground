...
import { Collapse, Icon, Select, Row, Col, Checkbox, Button, Table, InputNumber, Modal, Card, Empty, Steps, Slider, Radio } from 'antd';
import $ from 'jquery';
...
export const VrViewListPanel = (props) => {
  ...
  const { vrMallId, vrModelList, productList, vrAssets, assemblyGuide, onDisplayAsset, onAssemblyAsset, onZoomInOut, onSaveAsImage, onAutoRotate, onSettingInfo, onAssemblyGuide } = props;
...
*/
  ...
  const getControlIcon = () => {
    return (
        <Row align='middle' style={{paddingTop: '10px', backgroundColor: '#99999e'}}>
          ...
          <Col span={3}>
            <div id='play_icon' style={{textAlign: 'center'}}><Button onClick={ handlePlayRotate } type="primary" shape="circle"  size="small" icon="play-circle" style={controlStyle}></Button></div>
            <div id='play_title' style={{fontSize: '9px', textAlign: 'center'}}>회전</div>
            <div id='pause_icon' style={{display: 'none', textAlign: 'center'}}><Button onClick={ handlePauseRotate } type="primary" shape="circle"  size="small" icon="pause-circle" style={controlStyle}></Button></div>
            <div id='pause_title' style={{display: 'none', fontSize: '9px', textAlign: 'center'}}>멈춤</div>
          </Col>
          <Col span={4}>
            ...
            <Modal
              ...
              <div style={{ margin: 'auto', textAlign: 'left' }} >
                ...
                <Row style={{ marginTop: 16 }}>
                  ...
                  <Col span={4}>
                    <Checkbox
                      checked={assemblyGuide}
                      onChange={(e) => onAssemblyGuide(e.target.checked)}
                    />
                  </Col>
                </Row>
              </div>
  ...
  return (
    <div>
      <div id="vrViewListPanel" className='vrview-container' style={{ height: "100vh" }}>
        <Collapse
          activeKey={[1, 2, 3, 4]}
        >
          ...
          <Panel header="조립하기"  key="2" extra={genAssemblyExtra()} style={{ minHeight: "90px", maxHeight: "15vh", overflow: "auto" }}>
            <Row>
              <Col lg={8} md={8} xs={24} style={{padding: '2px'}}>
                <Slider
                  min={0}
                  max={maxLevel}
              ...
              <Col lg={16} md={16} xs={24} style={{padding: '2px'}}>
                <Col style={{marginLeft: 16}}>
                   <Button key="before_page" type='primary' icon='left' onClick={() => handleAssemblyAsset(current - 1)} style={{marginRight: "10px"}} />
                   [<span>{current}</span> / <span>{maxLevel}</span>]
                   <Button key="next_page" type='primary' icon='right' onClick={() => handleAssemblyAsset(current + 1)} style={{marginLeft: "10px"}} />
                 </Col>
                <InputNumber
                  min={0}
                  max={maxLevel}
          ...
          <Panel header="부품목록"  key="3" extra={genModifyExtra()} style={{ maxHeight: "80vh", overflow: "auto" }} >
