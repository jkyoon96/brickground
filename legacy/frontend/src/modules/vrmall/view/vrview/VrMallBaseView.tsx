...
import { VrViewPropertyPanel, VrViewListBasePanel, VrViewYoutubeBasePanel, ManualBookView } from '.';
...
export const VrMallBaseView = (props) => {
  ...
  */
  ...
  const initVrControls = () => {
 		...
 		var canvasWidth = $('#vrBaseView_vrCanvas').width();
 		var canvasHeight = $('#vrBaseView_vrCanvas').height() ;
    ...
    $('#vrBaseView_vrCanvas').on("mousedown", onDocumentMouseDown);
    //$('#vrBaseView_vrCanvas').on("click", onDocumentMouseClick);
    $('#vrBaseView_vrCanvas').on("dblclick", onDocumentMouseDblClick);
    //$('#vrBaseView_vrCanvas').on("touchend", onDocumentTouchClick);
    $('#vrBaseView_vrCanvas').on("display_asset", onDocumentDisplayAsset);
    $('#vrBaseView_vrCanvas').on("assembly_asset", onDocumentAssemblyAsset);
    ...
    if(navigator.platform) {
      if(filter.indexOf(navigator.platform.toLowerCase()) < 0) {
        if (window.orientation == 0)
          $('#vrBaseView_vrCanvas').css('height', '70vh');
        else
          $('#vrBaseView_vrCanvas').css('height', '100vh');
      }
    }
...
*/
  ...
  const handleDisplayAsset = (selectedModel: any, checked: boolean) => {
    ...
    var element = document.getElementById('vrBaseView_vrCanvas');
    if( element != null)
...
return (
            <Row gutter={[0,0]} style={{ position: 'fixed', top: '0px', left:'0px', bottom: '-10px',  width:'100vw', overflow: 'auto', opacity: '1' }} >
              ...
              <Col xxl={4} xl={6} lg={8} md={10} sm={24} style={{height:'100vh'}}>
                <VrViewListBasePanel   vrModelList={vrModelList}
